...
\begin{document}
	...
	\begin{equation}
		\theta(t) = \angle z_{FM}(t) = \arctan\left[\frac{\tilde{x}_{FM}(t)}{x_{FM}(t)}\right],
	\end{equation}
	que, após ser \emph{desembrulhada} (\emph{unwrapped}, para remover as descontinuidades múltiplas de $2\pi$ introduzidas pela função $\arctan$ numérica), corresponde exatamente ao argumento de $\sin(\cdot)$ em \eqref{eq:FM}. A frequência instantânea é obtida pela derivada da fase:
	\begin{equation}
	...
	\begin{enumerate}
		...
		\textbf{Dica:} a integral em \eqref{eq:FM} pode ser aproximada numericamente por uma soma cumulativa (\texttt{numpy.cumsum}) multiplicada pelo intervalo de amostragem $\Delta t$.
