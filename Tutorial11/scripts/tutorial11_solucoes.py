...
f1, Sxx_1 = periodograma(x_passafaixa[:N], fs, normalizacao='densidade')
f2, Sxx_2 = periodograma(x_passafaixa[:2*N], fs, normalizacao='densidade')
f5, Sxx_5 = periodograma(x_passafaixa[:5*N], fs, normalizacao='densidade')
f10, Sxx_10 = periodograma(x_passafaixa[:10*N], fs, normalizacao='densidade')

plt.figure(figsize=(8, 6))
plt.semilogx(f10, 10*np.log10(Sxx_10), ':',
             label=f'N={N} amostras (df={f1[1]:.1f} Hz)')
plt.semilogx(f5, 10*np.log10(Sxx_5), '-.',
             label=f'N={2*N}  amostras (df={f2[1]:.1f} Hz)')
plt.semilogx(f2, 10*np.log10(Sxx_2), '--',
             label=f'N={5*N} amostras (df={f5[1]:.1f} Hz)')
plt.semilogx(f1, 10*np.log10(Sxx_1),
             label=f'N={10*N} amostras (df={f10[1]:.1f} Hz)')
plt.grid()
plt.legend()
...
f1w, Sxxw_1 = welch(x_passafaixa[:N], Ndft, N_sobreposicao,
                       fs=fs, normalizacao='densidade')
f2w, Sxxw_2 = welch(x_passafaixa[:2*N], Ndft, N_sobreposicao,
                       fs=fs, normalizacao='densidade')
f5w, Sxxw_5 = welch(x_passafaixa[:5*N], Ndft, N_sobreposicao,
                       fs=fs, normalizacao='densidade')
f10w, Sxxw_10 = welch(x_passafaixa[:10*N], Ndft, N_sobreposicao,
                       fs=fs, normalizacao='densidade')

plt.figure(figsize=(8, 6))
plt.semilogx(f1w, 10*np.log10(Sxxw_1), '-.',
             label=f'N={N} amostras (df={f1w[1]:.1f} Hz)')
plt.semilogx(f2w, 10*np.log10(Sxxw_2), '-.',
             label=f'N={2*N} amostras (df={f2w[1]:.1f} Hz)')
plt.semilogx(f5w, 10*np.log10(Sxxw_5), '--',
             label=f'N={5*N} amostras (df={f5w[1]:.1f} Hz)')
plt.semilogx(f10w, 10*np.log10(Sxxw_10),
             label=f'N={10*N} amostras (df={f10w[1]:.1f} Hz)')
plt.grid()
plt.legend(loc='lower left')
plt.ylabel("Magnitude")
plt.title(f"Densidade Espectral de Potencia - Metodo de Welch (Ndft = {Ndft} amostras)")
plt.xlim([10, fs/2])
plt.ylim([-100, -25])
plt.xlabel("Frequencia [Hz]")
...
plt.loglog(f1_soma, PSD_soma1, '-',
             label = f"Ndft={Ndft} amostras")
plt.loglog(f2_soma, PSD_soma2, '--',
             label = f"Ndft={2*Ndft} amostras")
plt.loglog(f5_soma, PSD_soma5,'-.', 
             label = f"Ndft={5*Ndft} amostras")
plt.loglog(f10_soma, PSD_soma10, ':',
             label = f"Ndft={10*Ndft} amostras")

# marcar a potencia do sinal aletorio com uma linha tracejada em Vrms^2
plt.hlines(2/fs, f1_soma[0], f1_soma[-1], colors='k', linestyles='--')
plt.text(4e3, (2/fs) * 1.5, rf"{2/fs:.2e} $V_{{rms}}^2/Hz$", fontsize=12)
...
plt.text(20, Vrms**2 * 1.5, rf"{Vrms**2} $V_{{rms}}^2$", fontsize=12)
