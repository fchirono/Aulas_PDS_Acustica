...
\begin{document}
	...
	Em um conversor analógico-digital (\textit{Analog-to-Digital Converter}, ADC), um sinal elétrico $x[n]$ de entrada é mapeado para um conjunto finito de valores discretos, denominados \textbf{níveis de quantização}. Para um conversor de $N_\text{bits}$ bits, o número total de níveis disponíveis é $2^{N_\text{bits}}$. Em sistemas bipolares (sinais com componentes positiva e negativa), um bit é reservado para o sinal, resultando em $2^{N_\text{bits}-1}$ níveis por polaridade. 
		
	Para um conversor bipolar com tensão máxima $V_\text{max}$ (isto é, que aceita tensões de entrada entre $-V_{max}$ e $+V_{max}$), o tamanho $\Delta v$ de cada nível de quantização (também chamado de ``bit menos significante'', ou \textit{Least Significant Bit} - LSB) é dado por:
	\begin{equation}
	...
	Neste tutorial utilizamos um quantizador do tipo \textit{midtread} (Lipshitz et al., 1992), cuja operação é descrita pela expressão:
	\begin{equation}
		x_Q[n] = \Delta v \left\lfloor \frac{x[n]}{\Delta v} + \frac{1}{2}
		\right\rfloor
		\label{eq:midtread}
	\end{equation}
	onde $x[n]$ é a amostra do sinal original e $\lfloor \cdot \rfloor$ denota a função piso (\textit{floor}).
	...
	Quando o sinal de entrada é simples (por exemplo, uma senoide de baixa amplitude) ou quando sua amplitude é comparável ao tamanho do LSB, o erro de quantização deixa de ser aleatório e torna-se \textbf{determinístico} e correlacionado com o sinal de entrada. Isso resulta em distorção harmônica audível e modulação de ruído indesejadas.
	
	Uma técnica eficaz para mitigar esse problema é o \textit{dithering}, que consiste em adicionar um ruído de baixa amplitude ao sinal antes da quantização, de forma a descorrelacionar o erro de quantização do sinal de entrada. Conforme descrito por Lipshitz et al.\ (1992):
	...
	Como exemplo, o \textit{dithering} de espectro triangular pode ser obtido pela soma de dois sinais de ruído aleatório independentes com distribuição uniforme, cada um com amplitude $2 \cdot \Delta v$ pico a pico. O sinal de \textit{dither} é somado ao sinal original \textit{antes} da quantização.
	...
	% --------------------------------------------------
	\subsection{Exercício 1 — Função de Quantização}
	\label{sec:ex1}
	\begin{enumerate}
...
\end{lstlisting} 
	...
	% --------------------------------------------------
	\subsection{Exercício 2 — Amplitude do Sinal e Saturação}
	\label{sec:ex2}
	...
	% --------------------------------------------------
	\subsection{Exercício 3 — \textit{Dithering} (OPCIONAL)}
	\label{sec:ex3}
...
\end{lstlisting} 
	...
	\begin{enumerate}
		...
		\item Compare os espectros do erro com e sem \textit{dithering}. Os picos harmônicos são reduzidos? O nível de piso do erro aumenta ou diminui? Discuta as vantagens e desvantagens do \emph{dithering}.
	...
	% --------------------------------------------------
	\subsection{Exercício 4 — Relação Sinal-Ruído (OPCIONAL)}
	\label{sec:ex4}
	
	A relação sinal-ruído (\textit{Signal-to-Noise Ratio}, SNR) de quantização para um conversor L-PCM ideal é aproximada pela expressão clássica:
	\begin{equation}
		\text{SNR} \approx 6,02 \cdot N + 1,76 \quad \text{[dB]},
		\label{eq:snr}
	\end{equation}
	válida para um sinal senoidal que ocupa toda a faixa dinâmica do conversor.
	
	\begin{enumerate}
		\item Usando os sinais gerados pelo script, estime a SNR numericamente para $N = 4, 8$, e $16$ bits. Para isso, calcule a potência do sinal original ($P_x$) e a potência do erro ($P_e$), ambas pela média do quadrado dos respectivos vetores, e compute $\text{SNR} = 10\log_{10}(P_x / P_e)$ [dB].
		
		\item Compare os valores obtidos com a previsão da Equação~\eqref{eq:snr}. O resultado coincide? Se não, explique as possíveis causas da discrepância (amplitude do sinal em relação à faixa dinâmica, número de bits, etc.).
