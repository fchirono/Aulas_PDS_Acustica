...
\title{\vspace{-2cm} Processamento Digital de Sinais e Aplicações em Acústica\\ Soluções para Tutorial 06 - Transformada de Fourier de Tempo Discreto (DTFT) e Transformada Discreta de Fourier (DFT)}
...
\subsection*{Tarefa 8}
Devido à periodicidade da DTFT, o espectro de amplitude se repete nos intervalos $[-3\pi, -\pi]$ e $[\pi,3\pi]$, conforme visto na Figura \ref{fig:DTFT_periodica}.

\begin{figure}[h!]
	\centering
	\includegraphics[width = \textwidth,clip = true, trim = 0.5cm 0.2cm 0.5cm 0.2cm]{DTFT_periodica.png}
	\vskip-14pt
	\caption{DTFT do sinal {\tt A1.wav} plotado no intervalo $[-3\pi,3\pi]$ rad/amostra.}
	\label{fig:DTFT_periodica}
\end{figure}
...
\begin{figure}[h!]
	...
	\subfloat[$N_\textrm{DFT}=L-1$]{
		\includegraphics[width = 0.33\textwidth]{DFT_Lmenos1.png}
	}
	~
	\subfloat[$N_\textrm{DFT}=L+1$]{
		\includegraphics[width = 0.33\textwidth]{DFT_Lmais1.png}
	}
